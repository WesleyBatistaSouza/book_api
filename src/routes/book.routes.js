...
bookRouter.post("/book", (req, res) => {
  const { title, author, qtdPage, available, genre } = req.body;

  const newBook = createBook(title, author, qtdPage, available, genre);
...
bookRouter.put("/book/edit/:id", (req, res) => {
  ...
  const editBook = updateBook(id, title, author, qtdPage, available, genre);
